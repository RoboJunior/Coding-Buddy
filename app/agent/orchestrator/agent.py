...
toolset = McpToolset(
    ...
    tool_filter=["get_agent_cards", "call_agent"],
)

# Initalizing the agent Observability
orchestrator_agent_tracer = OpikTracer(
    name="orchestrator",
    tags=["deligate", "master"],
    metadata={
...
orchestrator_agent = LlmAgent(
    ...
    instruction="""
    You are CodingBuddy Orchestrator, an intelligent coordinator.
    Get all the agent cards and choice the right now and delgate the work to that agent.
    """,
    tools=[toolset],
    disallow_transfer_to_parent=True,
    disallow_transfer_to_peers=True,
    before_agent_callback=orchestrator_agent_tracer.before_agent_callback,
    after_agent_callback=orchestrator_agent_tracer.after_agent_callback,
    before_model_callback=orchestrator_agent_tracer.before_model_callback,
    after_model_callback=orchestrator_agent_tracer.after_model_callback,
    before_tool_callback=orchestrator_agent_tracer.before_tool_callback,
    after_tool_callback=orchestrator_agent_tracer.after_tool_callback,
)
